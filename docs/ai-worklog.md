...
## 2026-03-11 - 前端与解析体验重构

- 日期：2026-03-11
- 任务摘要：重构阅读器前端布局与样式，新增章节搜索、高亮当前章节、可见章节信息、段距/宽度设置；同时优化 TXT 章节识别与兜底分段体验。
- 版本号：N/A
- 验证命令结果：`npm run compile && npm test`（10 passing）
- Release 链接：N/A
- Commit Hash：待提交
- 人工验收：是（涉及 UI / 交互）
- 备注：建议本地安装后重点验证：目录搜索、章节切换、TXT 导入后段落观感。
